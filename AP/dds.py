...
scanned_path = home+'/framework/scanned'
...
def quitGracefully(clear=True):
    try:
        if clear == True:
            print('')
        os.system('stty sane') 
        print(f'\n{LIGHTGRAY}Thank you for using {CYAN}The Framework{LIGHTGRAY}.\n'\
        ...
        print(f'\n{LIGHTGRAY}Thank you for using the Framework')
    print(f"{LIGHTBLUE}* {GREEN}Take Care Bro!!!")
...
def selectInterface():
    ...
    while True:
        try:
            ...
            interface = int(input(f"\n{LIGHTGRAY}Select wifi {CYAN}interface ID {LIGHTGRAY}for {CYAN}monitoring {LIGHTGRAY}by number > {LIGHTORANGE}"))
